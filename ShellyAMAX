...
/* 
	log(`setState.Armed:
	${config.AMAX.setState.Armed.name}
	${config.AMAX.setState.Armed.id}
	${config.AMAX.setState.Armed.ACTIVE}
	setState.Disarmed:
	${config.AMAX.setState.Disarmed.name}
	${config.AMAX.setState.Disarmed.id}
	${config.AMAX.setState.Disarmed.ACTIVE}
	getState.Armed:
	${config.AMAX.getState.Armed.name}
	${config.AMAX.getState.Armed.id}
	${config.AMAX.getState.Armed.ACTIVE}
	getState.Triggered:
	${config.AMAX.getState.Triggered.name}
	${config.AMAX.getState.Triggered.id}
	${config.AMAX.getState.Triggered.ACTIVE}
	getState.AltTriggered:
	${config.AMAX.getState.AltTriggered.name}
	${config.AMAX.getState.AltTriggered.id}
	${config.AMAX.getState.AltTriggered.ACTIVE}
	`)
*/ 
...
function decodeAMAX( message, info, output ) { 
	if (config.logMqtt) {log(`decoding : [${info.property}] with message [${message}]`);}
	let msg = JSON.parse(message).state, d = Date.now();
	if (config.ShellyGen == 1) {msg = message;}
	...
	if (info.property == "currentState") {
		...
		if (msg == config.AMAX.getState.Triggered.ACTIVE ||
			...
			if (config.logMqtt) {log(`Notifying currentState = ${current_state_id}`);}
